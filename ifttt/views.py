...
def notify(clause):
    ...
    response = requests.post(url, data=json.dumps(post_data), headers={"IFTTT-Service-Key": key, "Content-Type": "application/json"})
...
def state(request):
    ...
    try:
        locals = {'should_trigger': False, 'state': state}
